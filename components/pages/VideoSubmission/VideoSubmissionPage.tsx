...
const VideoSubmissionPage: React.FC = () => {
  ...
  }, [user?.id, setEligibilityStatus, setIsCheckingEligibility]);

  useEffect(() => {
    checkSubmissionEligibility();
  }, [user?.id, checkSubmissionEligibility]);
        ...
        <div className="bg-black min-h-screen py-16 px-4 sm:px-6 lg:px-8">
          <div className="max-w-xl mx-auto">
            ...
            <div className="mt-6 flex justify-center space-x-4">
                              <LinkButton href="/login">{t('noAuth.login')}</LinkButton>
                <LinkButton href="/create-account" variant="outline">{t('noAuth.createAccount')}</LinkButton>
            </div>
          </div>
        </div>
      </Layout>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-2 bg-gray-700 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#9b9b6f] disabled:opacity-50"
                    >

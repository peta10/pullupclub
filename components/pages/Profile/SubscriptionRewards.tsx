...
const SubscriptionRewards: React.FC = () => {
  ...
  const calculatePatchProgress = useCallback(async () => {
    if (!user?.id) return;
  ...
  }, [user?.id, setWeeklyEarnings, setPayoutRequests, setPaypalEmail, setError, setIsLoading]);

  useEffect(() => {
    fetchEarnings();
    calculatePatchProgress();
  }, [user?.id, fetchEarnings, calculatePatchProgress]);
    ...
    <div className="space-y-8">
      ...
      {showPaypalModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" data-lenis-prevent>
          <div className="bg-gray-900 rounded-lg p-6 w-full max-w-md" data-lenis-prevent>

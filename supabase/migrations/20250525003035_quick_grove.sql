...
-- Add RLS policies to tables (not materialized views)
...
-- Create a secure view function to access badge statistics with RLS
CREATE OR REPLACE FUNCTION public.get_badge_statistics()
RETURNS SETOF public.badge_statistics
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Access denied: Only admins can access badge statistics';
    END IF;

    RETURN QUERY
    SELECT * FROM public.badge_statistics;
END;
$$;

-- Revoke direct access to the materialized view
REVOKE ALL ON public.badge_statistics FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_badge_statistics() TO authenticated;
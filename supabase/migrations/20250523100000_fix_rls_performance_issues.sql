...
-- Combined SELECT policy
CREATE POLICY "Combined email_events select policy" ON public.email_events
    FOR SELECT
    TO public
    USING (
        ((SELECT auth.uid()) = user_id) OR
...
-- Combined SELECT policy
CREATE POLICY "Combined email_engagement select policy" ON public.email_engagement
    FOR SELECT
    TO public
    USING (
        ((SELECT auth.uid()) = user_id) OR
...
-- Combined SELECT policy
CREATE POLICY "Combined email_suppression select policy" ON public.email_suppression
    FOR SELECT
    TO public
    USING (
        ((SELECT auth.uid()) = user_id) OR
...
-- Create optimized policies for dependencies table
...
-- ============================================
-- FIX DEPENDENCY_UPDATES TABLE POLICIES
-- ============================================

-- Drop existing policies
DROP POLICY IF EXISTS "Admin can view dependency updates" ON public.dependency_updates;

-- Create optimized policies for dependency_updates table

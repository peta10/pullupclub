import { products, productIds } from "./stripe-config";
...
export async function createCheckoutSession(
  ...
  try {
    ...
    const requestBody = {
      priceId,
      customerEmail,
      successUrl: `${typeof window !== 'undefined' ? window.location.origin : 'https://pullupclub.com'}/success?checkout=completed&plan=${plan}`,
    ...
    // Call Next.js API route which proxies to Supabase edge function
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error || 'Checkout API request failed');
    }
...
export async function createCustomerPortalSession(): Promise<string | null> {
  try {
    const { data, error } = await supabase.functions.invoke('customer-portal', {
      body: {
        returnUrl: `${typeof window !== 'undefined' ? window.location.origin : 'https://pullupclub.com'}/profile`,
      },
    });
